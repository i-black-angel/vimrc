...
let company="广东南方数码科技有限公司测绘软件事业部"

let g:template['c']['head'] = "/*\<cr>Copyright (c) 2009, ".company."\<cr>".
      \"All rights reserved.\<cr>\<cr>".
      \"文件名称：\<c-r>=GetNormalFileName()\<cr>\<cr>".
      \"摘    要：".g:rs."简要介绍文件内容".g:re."\<cr>\<cr>当前版本：".g:rs."...".g:re."\<cr>".
      \"创 建 者：".name."\<cr>邮    箱：".mail."\<cr>".
      \"创建日期：\<c-r>=strftime(\"%Y-%m-%d %H:%M:%S\")\<cr>\<cr>\<left>/"
let g:template['c']['coc'] = "/*\<cr>类 名：".g:rs."...".g:re."\<cr>".
      \"命名空间：".g:rs."...".g:re."\<cr>".
      \"描 述：".g:rs."...".g:re."\<cr>\<cr>".
      \"创建者：".name."\<cr>".
      \"邮  箱：".mail."\<cr>".
      \"创建日期：\<c-r>=strftime(\"%Y-%m-%d %H:%M:%S\")\<cr>\<cr>\<left>/"
let g:template['c']['cof'] = "/*\<cr>函数名：".g:rs."function name".g:re."\<cr>".
      \"函数功能：".g:rs."...".g:re."\<cr>".
      \"调    用：".g:rs."被本函数调用的函数清单".g:re."\<cr>".
      \"被 调 用：".g:rs."调用本函数的函数清单".g:re."\<cr>".
      \"输入参数：".g:rs."...".g:re."\<cr>".
      \"输出参数：".g:rs."...".g:re."\<cr>".
      \"函数返回值：".g:rs."...".g:re."\<cr>\<cr>".
      \"创建者：".name."\<cr>".
      \"邮箱：".mail."\<cr>".
      \"创建日期：\<c-r>=strftime(\"%Y-%m-%d %H:%M:%S\")\<cr>\<cr>\<left>/"
